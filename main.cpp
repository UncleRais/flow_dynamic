...
Vector solve(const problem_params &ps, bool file_logging = true, bool tests = true) {
    ...
    for (size_type step = 0; step < time_steps; ++step) {
        ...
        if (file_logging) ps.export_results(sol_prev, velocity_u, velocity_v, filename + ps.str_i(step + 1));
    ...
    utl::timer::start();
    if (file_logging) ps.export_vtu_series("timesteps");
